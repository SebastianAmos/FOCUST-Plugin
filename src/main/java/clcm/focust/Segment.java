 ...
 * Three segmentation protcols currently available (processSpheroids,
 * processSingleCells and processSpeckles.
 * 
...
public class Segment {
			...
			public void run() {
				...
				for (String file:filesList) {
					...
					if (analysisOnly) {
						...
						secondaryObjectsSpeckles = gpuSegmentGreaterConstant(channelsSpeckle[secondaryChannelChoice],
						...
						tertiaryObjectsSpeckles = gpuSegmentGreaterConstant(channelsSpeckle[tertiaryChannelChoice],
	...
	/**
	 * TODO: - implement kill borders function for secondary and primary objects
	 * (mask primary by secondary for instances where secondary objects are removed,
	 * so that child objects are as well). - Multiple primary object support per
	 * secondary object. -
	 */

	/**
	 * This method segments primary and secondary objects from "single cell"
	 * datasets based on user-defined parameters. Primary (nuclei) and secondary
	 * (cells) objects are used to create tertiary (cytoplasmic) labels. Primary,
	 * secondary and tertiary metrics are then related so each row in the final
	 * results table all pertain to the same biological entity. Support for multiple
	 * primary objects per secondary object coming...
	 * 
	 * @param analysisOnly Boolean from gui checkbox to determine if segmentation
		...
		Thread t1 = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < list.length; i++) {
					...
					// TESTING!!
					// IJ.saveAs(primaryOriginalObjectsCells, "TIF", dir +
					// "Primary_Original_Objects_" + imgName);
					// for some reason saving as a binary output, not a connected component label
					// map.
					////
					...
					// If analysis-only-mode, find the right secondary object file for the current
					// image.
					if (analysisOnly) {
						String fileName = list[i].replace(".nd2", ".tif");
						 ...
						 * IJ.error("Error",
						 * "The raw image and secondary object stack sizes do not match."); }
						 */
					} else {
						secondaryObjectsCells = gpuSegmentGreaterConstant(channelsSingleCell[secondaryChannelChoice],
								SingleCellView.sigma_x2, SingleCellView.sigma_y2, SingleCellView.sigma_z2,
								SingleCellView.greaterConstantSecondary, SingleCellView.radius_x2,
								SingleCellView.radius_y2, SingleCellView.radius_z2);
					}
					...
					// TESTING!!
					// IJ.saveAs(secondaryObjectsCells, "TIF", dir + "Secondary_Original_Objects_" +
					// imgName);

					// Give primary objects the same label ID as the secondary objects
					// Convert to binary, get max range value (maxLUT), divide label IDs by maxLUT
					// so ID = 1.

					// ImagePlus primaryLabelMatch = primaryOriginalObjectsCells.duplicate();
					// IJ.run(primaryLabelMatch, "Make Binary", "method=Huang background=Dark
					// black");
					...
					IJ.run(primaryLabelMatch, "Divide...",
							"value=" + primaryLabelMatch.getDisplayRangeMax() + " stack");
					// IJ.saveAs(primaryLabelMatch, "TIF", dir + "objectvaluesshouldbe1_" +
					// imgName);

					// Assign secondary labels to primary objects
					ImagePlus primaryObjectsCells = ImageCalculator.run(secondaryObjectsCells, primaryLabelMatch,
							"Multiply create stack");
					// IJ.saveAs(primaryObjectsCells, "TIF", dir + "objectvaluesshouldbesecondary!_"
					// + imgName);

					// Generate a tertiary (cytoplasmic) ROI (subtract primary from secondary
					// objects)
					IJ.log("Processing Tertiary Object...");
					ImagePlus tertiaryObjectsCells = ImageCalculator.run(secondaryObjectsCells, primaryObjectsCells,
							"Subtract create stack");

					// set calibration and save the ROI if the - assuming analysis only mode provide
					// primary and secondary objects only!
					...
					/*
					 * REMAP THE LABELS for the primary objects!!!!!! - Where two primary objects
					 * share a label acquired from a secondary object, but were previously uniquely
					 * identified, append an index to the shared label values to induvidualise them
					 * again. - Make this an option (maybe another check box?)
					 */
					 ...
					 *
					 * - primaryObjectsCell contains the labelling from the secondary objects. Some
					 * primary objects may share the same label. - primaryOriginalObjectsCells
					 * contains the original labels that uniquely identify each primary object.
					 * 
					 */
					...
					/*
					 * // THE OUTPUT IS MOSTLY BINARY BUT ALL HIGH-Z-VALUE SMALL SPECKS ARE
					 * MULTI-LABELLED! ImagePlus originalLabs =
					 * primaryOriginalObjectsCells.duplicate(); ImagePlus matchedLabs1 =
					 * primaryObjectsCells.duplicate(); ImagePlus relabelled1 =
					 * LabelEditor.manageDuplicates(matchedLabs1, originalLabs);
					...
					// If analysis mode is false, save the segmented outputs for the primary and
					// secondary object channels, then the tertiary object output.
					if (!analysisOnly) {
					...
					ResultsTable primarySCC1Intensity = TableUtility.processIntensity(channelsSingleCell[0],
							primaryObjectsCells);
					...
					if (numberOfChannels >= 2) {
						primarySCC2Intensity = TableUtility.processIntensity(channelsSingleCell[1],
								primaryObjectsCells);
					}
					if (numberOfChannels >= 3) {
						primarySCC3Intensity = TableUtility.processIntensity(channelsSingleCell[2],
								primaryObjectsCells);
					}
					if (numberOfChannels >= 4) {
						primarySCC4Intensity = TableUtility.processIntensity(channelsSingleCell[3],
								primaryObjectsCells);
					}
					...
					// check to see if grouping info has been entered, if yes, then populate the
					// imageData table, if not, ignore.
					...
					primaryFinalTable.setColumn("C1_Mean_Intensity",
							primarySCC1Intensity.getColumnAsVariables("Mean_Intensity"));
					primaryFinalTable.setColumn("C1_IntDen", primarySCC1Intensity.getColumnAsVariables("IntDen"));
					if (numberOfChannels >= 2) {
						primaryFinalTable.setColumn("C2_Mean_Intensity",
								primarySCC2Intensity.getColumnAsVariables("Mean_Intensity"));
						primaryFinalTable.setColumn("C2_IntDen", primarySCC2Intensity.getColumnAsVariables("IntDen"));
					}
					if (numberOfChannels >= 3) {
						primaryFinalTable.setColumn("C3_Mean_Intensity",
								primarySCC3Intensity.getColumnAsVariables("Mean_Intensity"));
						primaryFinalTable.setColumn("C3_IntDen", primarySCC3Intensity.getColumnAsVariables("IntDen"));
					}
					if (numberOfChannels >= 4) {
						primaryFinalTable.setColumn("C4_Mean_Intensity",
								primarySCC4Intensity.getColumnAsVariables("Mean_Intensity"));
					...
					ResultsTable secondarySCC1Intensity = TableUtility.processIntensity(channelsSingleCell[0],
							secondaryObjectsCells);
					ResultsTable secondarySCC2Intensity = null;
					ResultsTable secondarySCC3Intensity = null;
					ResultsTable secondarySCC4Intensity = null;

					if (numberOfChannels >= 2) {
						secondarySCC2Intensity = TableUtility.processIntensity(channelsSingleCell[1],
								secondaryObjectsCells);
					}
					if (numberOfChannels >= 3) {
						secondarySCC3Intensity = TableUtility.processIntensity(channelsSingleCell[2],
								secondaryObjectsCells);
					}
					if (numberOfChannels >= 4) {
						secondarySCC4Intensity = TableUtility.processIntensity(channelsSingleCell[3],
								secondaryObjectsCells);
					}
					...
					secondaryFinalTable.setColumn("C1_Mean_Intensity",
							secondarySCC1Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("C1_IntDen", secondarySCC1Intensity.getColumnAsVariables("IntDen"));

					if (numberOfChannels >= 2) {
						secondaryFinalTable.setColumn("C2_Mean_Intensity",
								secondarySCC2Intensity.getColumnAsVariables("Mean_Intensity"));
						secondaryFinalTable.setColumn("C2_IntDen",
								secondarySCC2Intensity.getColumnAsVariables("IntDen"));
					}

					if (numberOfChannels >= 3) {
						secondaryFinalTable.setColumn("C3_Mean_Intensity",
								secondarySCC3Intensity.getColumnAsVariables("Mean_Intensity"));
						secondaryFinalTable.setColumn("C3_IntDen",
								secondarySCC3Intensity.getColumnAsVariables("IntDen"));
					}

					if (numberOfChannels >= 4) {
						secondaryFinalTable.setColumn("C4_Mean_Intensity",
								secondarySCC4Intensity.getColumnAsVariables("Mean_Intensity"));
						secondaryFinalTable.setColumn("C4_IntDen",
								secondarySCC4Intensity.getColumnAsVariables("IntDen"));
					}
					...
					ResultsTable tertiarySCC1Intensity = TableUtility.processIntensity(channelsSingleCell[0],
							tertiaryObjectsCells);
					ResultsTable tertiarySCC2Intensity = null;
					ResultsTable tertiarySCC3Intensity = null;
					ResultsTable tertiarySCC4Intensity = null;

					if (numberOfChannels >= 2) {
						tertiarySCC2Intensity = TableUtility.processIntensity(channelsSingleCell[1],
								tertiaryObjectsCells);
					}
					if (numberOfChannels >= 3) {
						tertiarySCC3Intensity = TableUtility.processIntensity(channelsSingleCell[2],
								tertiaryObjectsCells);
					}
					if (numberOfChannels >= 4) {
						tertiarySCC4Intensity = TableUtility.processIntensity(channelsSingleCell[3],
								tertiaryObjectsCells);
					}
					...
					tertiaryFinalTable.setColumn("C1_Mean_Intensity",
							tertiarySCC1Intensity.getColumnAsVariables("Mean_Intensity"));
					tertiaryFinalTable.setColumn("C1_IntDen", tertiarySCC1Intensity.getColumnAsVariables("IntDen"));

					if (numberOfChannels >= 2) {
						tertiaryFinalTable.setColumn("C2_Mean_Intensity",
								tertiarySCC2Intensity.getColumnAsVariables("Mean_Intensity"));
						tertiaryFinalTable.setColumn("C2_IntDen", tertiarySCC2Intensity.getColumnAsVariables("IntDen"));
					}

					if (numberOfChannels >= 3) {
						tertiaryFinalTable.setColumn("C3_Mean_Intensity",
								tertiarySCC3Intensity.getColumnAsVariables("Mean_Intensity"));
						tertiaryFinalTable.setColumn("C3_IntDen", tertiarySCC3Intensity.getColumnAsVariables("IntDen"));
					}

					if (numberOfChannels >= 4) {
						tertiaryFinalTable.setColumn("C4_Mean_Intensity",
								tertiarySCC4Intensity.getColumnAsVariables("Mean_Intensity"));
					...
					/*
					 * Build the final results table. Match labels between primary, secondary and
					 * tertiary results tables so that row label[x] = all data on x
					 */

					// TODO: add user-input channel names to the combined results table if the user
					// input text, otherwise leave as "Cx".
					...
					for (int j = 0; j < primaryFinalTable.size(); j++) {
						...
						// where row labels match between all three tables, add a row to the combined
						// results table
						if (secondaryRowIndex >= 0 && tertiaryRowIndex >= 0) {
							...
							combinedResults.addValue("Primary_Voxel_Count",
									primaryFinalTable.getValue("Voxel_Count", j));
							...
							combinedResults.addValue("Primary_Mean_Breadth",
									primaryFinalTable.getValue("Mean_Breadth", j));
							combinedResults.addValue("Primary_Surface_Area",
									primaryFinalTable.getValue("Surface_Area", j));
							...
							combinedResults.addValue("Primary_C1_Mean_Intensity",
									primaryFinalTable.getValue("C1_Mean_Intensity", j));
							combinedResults.addValue("Primary_C1_IntDen", primaryFinalTable.getValue("C1_IntDen", j));
							if (numberOfChannels >= 2) {
								combinedResults.addValue("Primary_C2_Mean_Intensity",
										primaryFinalTable.getValue("C2_Mean_Intensity", j));
								combinedResults.addValue("Primary_C2_IntDen",
										primaryFinalTable.getValue("C2_IntDen", j));
							}
							if (numberOfChannels >= 3) {
								combinedResults.addValue("Primary_C3_Mean_Intensity",
										primaryFinalTable.getValue("C3_Mean_Intensity", j));
								combinedResults.addValue("Primary_C3_IntDen",
										primaryFinalTable.getValue("C3_IntDen", j));
							}
							if (numberOfChannels >= 4) {
								combinedResults.addValue("Primary_C4_Mean_Intensity",
										primaryFinalTable.getValue("C4_Mean_Intensity", j));
								combinedResults.addValue("Primary_C4_IntDen",
										primaryFinalTable.getValue("C4_IntDen", j));
							}

							combinedResults.addValue("Secondary_Volume",
									secondaryFinalTable.getValue("Volume", secondaryRowIndex));
							combinedResults.addValue("Secondary_Voxel_Count",
									secondaryFinalTable.getValue("Voxel_Count", secondaryRowIndex));
							combinedResults.addValue("Secondary_Sphericity",
									secondaryFinalTable.getValue("Sphericity", secondaryRowIndex));
							combinedResults.addValue("Secondary_Elongation",
									secondaryFinalTable.getValue("Elongation", secondaryRowIndex));
							combinedResults.addValue("Secondary_Mean_Breadth",
									secondaryFinalTable.getValue("Mean_Breadth", secondaryRowIndex));
							combinedResults.addValue("Secondary_Surface_Area",
									secondaryFinalTable.getValue("Surface_Area", secondaryRowIndex));
							combinedResults.addValue("Secondary_Centroid_X",
									secondaryFinalTable.getValue("Centroid_X", secondaryRowIndex));
							combinedResults.addValue("Secondary_Centroid_Y",
									secondaryFinalTable.getValue("Centroid_Y", secondaryRowIndex));
							combinedResults.addValue("Secondary_Centroid_Z",
									secondaryFinalTable.getValue("Centroid_Z", secondaryRowIndex));
							combinedResults.addValue("Secondary_C1_Mean_Intensity",
									secondaryFinalTable.getValue("C1_Mean_Intensity", secondaryRowIndex));
							combinedResults.addValue("Secondary_C1_IntDen",
									secondaryFinalTable.getValue("C1_IntDen", secondaryRowIndex));
							if (numberOfChannels >= 2) {
								combinedResults.addValue("Secondary_C2_Mean_Intensity",
										secondaryFinalTable.getValue("C2_Mean_Intensity", secondaryRowIndex));
								combinedResults.addValue("Secondary_C2_IntDen",
										secondaryFinalTable.getValue("C2_IntDen", secondaryRowIndex));
							}
							if (numberOfChannels >= 3) {
								combinedResults.addValue("Secondary_C3_Mean_Intensity",
										secondaryFinalTable.getValue("C3_Mean_Intensity", secondaryRowIndex));
								combinedResults.addValue("Secondary_C3_IntDen",
										secondaryFinalTable.getValue("C3_IntDen", secondaryRowIndex));
							}
							if (numberOfChannels >= 4) {
								combinedResults.addValue("Secondary_C4_Mean_Intensity",
										secondaryFinalTable.getValue("C4_Mean_Intensity", secondaryRowIndex));
								combinedResults.addValue("Secondary_C4_IntDen",
										secondaryFinalTable.getValue("C4_IntDen", secondaryRowIndex));
							}
							combinedResults.addValue("Tertiary_Volume",
									tertiaryFinalTable.getValue("Volume", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Voxel_Count",
									tertiaryFinalTable.getValue("Voxel_Count", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Sphericity",
									tertiaryFinalTable.getValue("Sphericity", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Elongation",
									tertiaryFinalTable.getValue("Elongation", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Mean_Breadth",
									tertiaryFinalTable.getValue("Mean_Breadth", tertiaryRowIndex));
							combinedResults.addValue("Surface_Area",
									tertiaryFinalTable.getValue("Surface_Area", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Centroid_X",
									tertiaryFinalTable.getValue("Centroid_X", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Centroid_Y",
									tertiaryFinalTable.getValue("Centroid_Y", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_Centroid_Z",
									tertiaryFinalTable.getValue("Centroid_Z", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_C1_Mean_Intensity",
									tertiaryFinalTable.getValue("C1_Mean_Intensity", tertiaryRowIndex));
							combinedResults.addValue("Tertiary_C1_IntDen",
									tertiaryFinalTable.getValue("C1_IntDen", tertiaryRowIndex));
							if (numberOfChannels >= 2) {
								combinedResults.addValue("Tertiary_C2_Mean_Intensity",
										tertiaryFinalTable.getValue("C2_Mean_Intensity", tertiaryRowIndex));
								combinedResults.addValue("Tertiary_C2_IntDen",
										tertiaryFinalTable.getValue("C2_IntDen", tertiaryRowIndex));
							}
							if (numberOfChannels >= 3) {
								combinedResults.addValue("Tertiary_C3_Mean_Intensity",
										tertiaryFinalTable.getValue("C3_Mean_Intensity", tertiaryRowIndex));
								combinedResults.addValue("Tertiary_C3_IntDen",
										tertiaryFinalTable.getValue("C3_IntDen", tertiaryRowIndex));
							}
							if (numberOfChannels >= 4) {
								combinedResults.addValue("Tertiary_C4_Mean_Intensity",
										tertiaryFinalTable.getValue("C4_Mean_Intensity", tertiaryRowIndex));
								combinedResults.addValue("Tertiary_C4_IntDen",
										tertiaryFinalTable.getValue("C4_IntDen", tertiaryRowIndex));
							}

							// create some ratios -> MAKE THIS CONDITIONAL ON USER INPUT!!! Modular in some
							// way?
							combinedResults.addValue("Primary/Tertiary_Volume_Ratio",
									primaryFinalTable.getValue("Volume", j)
											/ tertiaryFinalTable.getValue("Volume", tertiaryRowIndex));
							if (numberOfChannels >= 2) {
								combinedResults.addValue("Primary/Tertiary_C2_IntDen_Ratio",
										primaryFinalTable.getValue("C2_IntDen", j)
												/ tertiaryFinalTable.getValue("C2_IntDen", tertiaryRowIndex));
							}
							if (numberOfChannels >= 3) {
								combinedResults.addValue("Primary/Tertiary_C3_IntDen_Ratio",
										primaryFinalTable.getValue("C3_IntDen", j)
												/ tertiaryFinalTable.getValue("C3_IntDen", tertiaryRowIndex));
							}
							if (numberOfChannels >= 4) {
								combinedResults.addValue("Primary/Teritary_C4_IntDen_Ratio",
										primaryFinalTable.getValue("C4_IntDen", j)
												/ tertiaryFinalTable.getValue("C4_IntDen", tertiaryRowIndex));
							}

							/*
							 * // loop through each for (int x = 1; x <= numberOfChannels; x++) { String
							 * table = "Primary"; String meanName = table + "_C" + Integer.toString(x) +
							 * "_Mean_Intensity"; String meanTable = "C" + Integer.toString(x) +
							 * "_Mean_Intensity"; String intdenName = table + "_C" + Integer.toString(x) +
							 * "_IntDen"; String intdenTable = "C" + Integer.toString(x) + "_IntDen";
							 * IJ.log(meanName + " and " + meanTable + " and " + intdenName + " and " +
							 * intdenTable);
							 * 
							 * combinedResults.addValue(meanName, primaryFinalTable.getValue(meanTable, j));
							 * combinedResults.addValue(intdenName, primaryFinalTable.getValue(intdenTable,
							 * j)); }
							 * 
							 * for (int y = 1; y <= numberOfChannels; y++) { String table = "Tertiary";
							 * String meanName = table + "_C" + Integer.toString(y) + "_Mean_Intensity";
							 * String meanTable = "C" + Integer.toString(y) + "_Mean_Intensity"; String
							 * intdenName = table + "_C" + Integer.toString(y) + "_IntDen"; String
							 * intdenTable = "C" + Integer.toString(y) + "_IntDen";
							 * 
							 * combinedResults.addValue(meanName, tertiaryFinalTable.getValue(meanTable,
							 * tertiaryRowIndex)); combinedResults.addValue(intdenName,
							 * tertiaryFinalTable.getValue(intdenTable, tertiaryRowIndex)); }
							 */
	...
	/**
	 * A helper method for matching labels between results tables in
	 * processSingleCells().
	 * 
	...
	// parameterise this!!! processSpheoroid(boolean analysisOnly, String inputDir,
	// String outputDir, int primaryChannelChoie, int secondaryChannelChoice etc.)
	/*
	 * -----------------------------------------------------------------------------
	 * ------- This method segments primary and secondary objects based on
	 * user-defined parameters. Objects are then used for region-restricted
	 * intensity analysis and 3D measurements.
	 * -----------------------------------------------------------------------------
	 * ------
	 */
	public static void processSpheroid(boolean analysisOnly) {
		Thread t1 = new Thread(new Runnable() {
			...
			public void run() {
				...
				// If analysis-only-mode, create a new list[] containing image names that DO NOT
				// match the prefix expectations i.e. are the original images, not the objects.
				if (analysisOnly) {
					ArrayList<String> tempList = new ArrayList<>();
					for (String imgName : list) {
						if (!imgName.startsWith(PRIMARYPREFIX) && !imgName.startsWith(SECONDARYPREFIX)
				...
				// Iterate through each image in the directory and segment the selected primary
				...
				for (int i = 0; i < list.length; i++) {
					...
					// If analysis-only-mode, find the right primary object file for the current
					// image.
					if (analysisOnly) {
						String fileName = list[i].replace(".nd2", ".tif");
						...
						primaryObjectSpheroid = gpuSegmentOtsu(channelsSpheroid[primaryChannelChoice],
								SpheroidView.sigma_x, SpheroidView.sigma_y, SpheroidView.sigma_z, SpheroidView.radius_x,
								SpheroidView.radius_y, SpheroidView.radius_z);
					}
					...
					/*
					 * // saving this just for testing (checking for row mis-match in the final
					 * table) String pRName = dir + "Primary_Results.csv"; // saving this just for
					 * testing (checking for row mis-match in the final table) try {
					 * primaryResults.saveAs(pRName); } catch (IOException e) { e.printStackTrace();
					...
					/*
					 * Create Spheroid ROIs - Binary search for the optimal number of 3D erosions
					 * that yeild the closest final volume target for the core (innerNew)
					 */
					...
					// Create outer ROI
					ImagePlus outerROI = ImageCalculator.run(secondaryObjectSpheroid, innerROI,
							"Subtract create stack");
					...
					// TODO: change c1 and c3 back from static. Unless best practice? That was just
					// part of testing.

					ResultsTable primaryC1Intensity = TableUtility.processIntensity(channelsSpheroid[0],
							primaryObjectSpheroid);
					...
					/*
					 * Write image name and grouping (where entered) data to a results table. This
					 * allows whole columns to be extracted as Variable[] to construct the final
					 * table.
					 */
					...
					/*
					 * TODO > Make this conditional: Not all images will contain 4 channels to
					 * analyse the intensities of - grab channel array and just run for each element
					 * creating a new resultstable each time?
					 */
					ResultsTable secondaryC1Intensity = TableUtility.processIntensity(channelsSpheroid[0],
							secondaryObjectSpheroid);
					ResultsTable secondaryC2Intensity = null;
					ResultsTable secondaryC3Intensity = null;
					ResultsTable secondaryC4Intensity = null;

					if (numberOfChannels >= 2) {
						secondaryC2Intensity = TableUtility.processIntensity(channelsSpheroid[1],
								secondaryObjectSpheroid);
					}
					if (numberOfChannels >= 3) {
						secondaryC3Intensity = TableUtility.processIntensity(channelsSpheroid[2],
								secondaryObjectSpheroid);
					}
					if (numberOfChannels >= 4) {
						secondaryC4Intensity = TableUtility.processIntensity(channelsSpheroid[3],
								secondaryObjectSpheroid);
					}
					...
					secondaryFinalTable.setColumn("Centroid_Y", secondaryResults.getColumnAsVariables("Centroid.Y"));
					secondaryFinalTable.setColumn("Centroid_Z", secondaryResults.getColumnAsVariables("Centroid.Z"));
					secondaryFinalTable.setColumn("Whole_C1_Mean_Intensity",
							secondaryC1Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Whole_C1_IntDen",
							secondaryC1Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Whole_C2_Mean_Intensity",
							secondaryC2Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Whole_C2_IntDen",
							secondaryC2Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Whole_C3_Mean_Intensity",
							secondaryC3Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Whole_C3_IntDen",
							secondaryC3Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Whole_C4_Mean_Intensity",
							secondaryC4Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Whole_C4_IntDen",
							secondaryC4Intensity.getColumnAsVariables("IntDen"));

					secondaryFinalTable.setColumn("Core_C1_Mean_Intensity",
							coreC1Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Core_C1_IntDen", coreC1Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Periphery_C1_Mean_Intensity",
							peripheryC1Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Periphery_C1_IntDen",
							peripheryC1Intensity.getColumnAsVariables("IntDen"));

					secondaryFinalTable.setColumn("Core_C2_Mean_Intensity",
							coreC2Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Core_C2_IntDen", coreC2Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Periphery_C2_Mean_Intensity",
							peripheryC2Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Periphery_C2_IntDen",
							peripheryC2Intensity.getColumnAsVariables("IntDen"));

					secondaryFinalTable.setColumn("Core_C3_Mean_Intensity",
							coreC3Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Core_C3_IntDen", coreC3Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Periphery_C3_Mean_Intensity",
							peripheryC3Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Periphery_C3_IntDen",
							peripheryC3Intensity.getColumnAsVariables("IntDen"));

					secondaryFinalTable.setColumn("Core_C4_Mean_Intensity",
							coreC4Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Core_C4_IntDen", coreC4Intensity.getColumnAsVariables("IntDen"));
					secondaryFinalTable.setColumn("Periphery_C4_Mean_Intensity",
							peripheryC4Intensity.getColumnAsVariables("Mean_Intensity"));
					secondaryFinalTable.setColumn("Periphery_C4_IntDen",
							peripheryC4Intensity.getColumnAsVariables("IntDen"));
	 ...
	 * 
	 * @param channel  ImagePlus of the channel to process
	 * @param sigma_x  The x sigma for Gaussian blur 3D
	 * @param sigma_y  The y sigma for Gaussian blur 3D
	 * @param sigma_z  The z sigma for Gaussian blur 3D
	 * @param detect_x The x radius of spot detection
	 * @param detect_y The y radius of spot detection
	 * @param detect_z The z radius of spot detection
	 * 
	 * @return A segmented ImagePlus.
	 */
	public static ImagePlus gpuSegmentOtsu(ImagePlus channel, double sigma_x, double sigma_y, double sigma_z,
			double detect_x, double detect_y, double detect_z) {
	...
	/**
	 * Spheroid secondary object (whole spheroid without segmentation) One spheroid
	 * per image only.
	 * 
	 ...
	 * @param sigma_y  The y sigma for Gaussian blur 3D
	 * @param sigma_z  The z sigma for Gaussian blur 3D
	 * @param constant The threshold constant value
	 * @param detect_x The x radius of spot detection
	 * @param detect_y The y radius of spot detection
	 * @param detect_z The z radius of spot detection
	 * 
	 * @return A segmented ImagePlus.
	 */
	public static ImagePlus gpuSegmentGreaterConstant(ImagePlus channel, double sigma_x, double sigma_y, double sigma_z,
			double constant, double detect_x, double detect_y, double detect_z) {
